...
install() {
    bash <(curl -Ls https://github.com/RandomNessan/v2bx_install_oneclick-20250725/blob/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://github.com/RandomNessan/v2bx_install_oneclick-20250725/blob/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://github.com/RandomNessan/v2bx_install_oneclick-20250725/blob/main/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
